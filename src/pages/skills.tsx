...
import { PRIME_COLOUR } from "src/configs";
...
const SkillPage = ({ data }: IProps) => {
  ...
  const tagItems = tags.map((_t) => {
    return (<div className="tag-span form-check">
      <input type="checkbox" className="form-check-input"
        id={`tag-${_t}`} checked={selectedTags[_t]}
        onChange={() => setSelectedTags({
          ...selectedTags,
          [_t]: !selectedTags[_t]
        })} />
      <label className="form-check-label" htmlFor={`tag-${_t}`}>
        {_t}
      </label>
    </div>);
  });
  ...
  const skillItems = skills.map((skillNode) => {
    ...
    const _frontSide = _sNode.backgroundImage ?
      <SkillImage path={`skills/${_sNode.backgroundImage}`} alt={`${_sNode.label}`} /> :
      (<h4 className="d-flex justify-content-center align-items-center text-center">
        {`${_sNode.label.toUpperCase()}`}
      </h4>);

    return (<div className="col-xl-3 col-lg-4 col-sm-6 col-12 flipy-col">
      <Flippy
  ...
  return (
    <Layout>
      <SEO title="Skills" />
      <div className="row row-skills-tags form-inline">
        {tagItems}
      </div>
